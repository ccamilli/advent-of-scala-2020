...
def excludePossibilities(row: (Set[Ingredient], Set[Allergen]),
...
def eliminatePossibilities(parsedInput: List[(Set[Ingredient], Set[Allergen])]): Map[Ingredient, Set[Allergen]] = {
  val allIngredients = parsedInput.map(_._1).reduce(_ ++ _)
  val exclRaw = parsedInput.flatMap(row => excludePossibilities(row, allIngredients))
  val excludedPossibilities = exclRaw.groupBy(x => x._1).map {
...
def solvePart1(parsedInput: List[(Set[Ingredient], Set[Allergen])]): Int = {
  ...
  val noAllergies = eliminatePossibilities(parsedInput).filter(x => x._2.size == allAllergens.size).keys.toSet
...
def solvePart2(parsedInput: List[(Set[Ingredient], Set[Allergen])]): String = {
  ...
  if (answer.size == fullAlergens.size) answer
    ...
    val stillPossible = eliminatePossibilities(currInput).map {
